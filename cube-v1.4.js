class Cube3D {
    ...
    async createTextures() {
        ...
        // Diese müssen mit Items gefüllt werden für echte Infinity-Rotation
        // Bei 2 Items: front=1, right=2, back=1, left=2 (abwechselnd)
        // Bei 3 Items: front=1, right=2, back=3, left=1
        // Bei 4+ Items: front=1, right=2, back=3, left=4
        ...
        this.cube.material = [
            new THREE.MeshBasicMaterial({ map: texRight, transparent: true }),  // right = Item 2
            new THREE.MeshBasicMaterial({ map: texLeft, transparent: true }),   // left = Item 4 (oder Item 2 bei 2 Items)
            new THREE.MeshBasicMaterial(solidMat),                              // top
            new THREE.MeshBasicMaterial(solidMat),                              // bottom
            new THREE.MeshBasicMaterial({ map: texFront, transparent: true }),  // front = Item 1
            new THREE.MeshBasicMaterial({ map: texBack, transparent: true })    // back = Item 3 (oder Item 1 bei 2 Items)
        ];
    }
    ...
    onClick(e) {
        ...
        // Berechne wie oft wir komplett durchrotiert sind (360° = 4 Seiten)
        const totalRotations = Math.floor(Math.abs(rawRot) / (Math.PI * 2));
        const facesPerRotation = 4; // Ein Würfel zeigt 4 Seiten pro 360°
        
        // Gesamtzahl der gesehenen Seiten
        const totalFacesSeen = totalRotations * facesPerRotation + faceIndex;
        
        // Item-Index berechnen (rotiert durch alle Items)
