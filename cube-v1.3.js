class Cube3D {
    ...
    async createTextures() {
        ...
        // Cube hat 3 sichtbare Seiten: front, right, left
        // Items rotierend verteilen: bei 2 Items → 1,2,1 / bei 1 Item → 1,1,1 / bei 3+ → 1,2,3
        const textures = [];
        ...
        // Verteilung: front=tex0 (Item 1), right=tex1 (Item 2 oder 1), left=tex2 (Item 3 oder 1)
        this.cube.material = [
            new THREE.MeshBasicMaterial({ map: tex1, transparent: true }), // right
            new THREE.MeshBasicMaterial({ map: tex2, transparent: true }), // left
            new THREE.MeshBasicMaterial(solidMat), // top
            new THREE.MeshBasicMaterial(solidMat), // bottom
            new THREE.MeshBasicMaterial({ map: tex0, transparent: true }), // front
    ...
    onClick(e) {
        ...
        if (norm >= 0 && norm < Math.PI * 0.5) {
            ...
            faceIndex = 2; // back wird zu front
        } else {
            faceIndex = 0; // left wird zu front (zurück zu Start)
        }
        
        // Berechne wie oft wir komplett durchrotiert sind
        const totalRotations = Math.floor(Math.abs(rawRot) / (Math.PI * 2));
        const facesPerRotation = 3; // Ein Würfel zeigt 3 Seiten pro 360°
        
        // Gesamtzahl der "Klicks" durch den Stack
        const totalFacesSeen = totalRotations * facesPerRotation + faceIndex;
        
        // Bei 2 Items: Seiten sind verteilt als Item1, Item2, Item1 → Mapping: [0,1,0]
        // Bei 1 Item: Alle Seiten zeigen Item1 → Mapping: [0,0,0]
        // Bei 3+ Items: Normal → Mapping: [0,1,2]
        ...
        if (currentItem && currentItem.link) {
            ...
            if (!link.startsWith('http') && !link.startsWith('/listings/')) {
                link = '/listings/' + link.replace(/^\//, '');
            }
